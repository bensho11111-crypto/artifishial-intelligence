...
const _p0 = new THREE.Vector3(0, -30, 50);
let _r    = _p0.length();
let _az   = Math.atan2(_p0.y, _p0.x);
let _el   = Math.asin(Math.max(-1, Math.min(1, _p0.z / _r)));
...
canvas.addEventListener('mousemove',  e => {
  ...
  _az -= dx * 0.002;
  _el -= dy * 0.002;
  _el  = Math.max(0.04, Math.min(Math.PI / 2 - 0.04, _el));
  _applyOrbit();
});
canvas.addEventListener('wheel', e => {
  _r *= 1 + e.deltaY * 0.001;
  _r  = Math.max(5, Math.min(500, _r));
  _applyOrbit();
});
